...
- Label scoring (`scoreLabels`) returns a probability per offered label, normalized over the offered labels only, with negatives clamped to zero; when no offered label carries any mass it fails with the parse-failure category instead of returning a uniform distribution, and it reports the mass the offered labels held before renormalization as `support`. Backends with token log-probabilities answer from one generated position with a single output token (method `Logprobs`); every other backend derives the answer from a schema-constrained JSON reply the model writes itself (method `Verbalized`), and a reply that names a label it gave no probability is a parse failure, never a certainty.
- A judgment over one state answers each question independently, retries a question whose labels were not observed once through the verbalized path (recording the method and emitting a notice), and reports a question that still fails in `failures` while the other answers stand. Hosted answers carry calibration `Claimed`; answers re-asked of a reasoning model carry method `Reasoning` with `escalated` set. Cached answers are keyed on the judgment's identity (backend plus checkpoint), so a replaced local model never reuses its predecessor's answers.
