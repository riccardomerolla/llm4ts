...
const workspace = Effect.suspend(() => makeNodeWorkspace(kitsRoot))

const packDir = (kit: string, name: string): string => `${kit}/packs/${name}`

const load = (kit: string, name: string): Effect.Effect<Pack, unknown> =>
  Effect.flatMap(workspace, (space) => loadPack(space, packDir(kit, name)))

/**
 * The shipped reference packs are the modernization flows' default
 * configuration (`LLM4TS_PACK`, resolved by name across the built-in kits).
...
describe.each(packs)("kits/$kit/packs/$name", (expected) => {
  it.effect("loads with the fields the modernization flows read", () =>
    Effect.gen(function* () {
      const pack = yield* load(expected.kit, expected.name)
  ...
  it.effect("points at a scaffold that ships with the repository", () =>
    Effect.gen(function* () {
      const pack = yield* load(expected.kit, expected.name)
      const scaffold = resolve(kitsRoot, packDir(expected.kit, expected.name), pack.scaffold ?? "")
...
describe("cobol coverage rules", () => {
  it.effect("match representative COBOL paragraphs and JCL steps", () =>
    Effect.gen(function* () {
      const pack = yield* load("mainframe-java", "cobol-springboot")
...
describe("pattern cards", () => {
  it.effect("load from the universal directory with usable match regexes", () =>
    Effect.gen(function* () {
      const cards = yield* loadPatternCards(yield* workspace, "mainframe-java/patterns")
  ...
  it.effect("load from a pack-local directory too", () =>
    Effect.gen(function* () {
      const cards = yield* loadPatternCards(
        yield* workspace,
        "mainframe-java/packs/cobol-kafka/patterns"
      )
